...
main_func()
{
	...
	while [[ "$num" -ge 10 && -e "../../func_tests/data/neg_${num}_args.txt" ]]; do
		echo "TEST ${num}:"
		
		# using wc command to count number of words
		argc=`wc --word < ../../func_tests/data/neg_${num}_args.txt`
		
		if [[ "$argc" -eq 3 || "$argc" -eq 4 ]]; then
			cat ../../func_tests/data/neg_${num}_args.txt | xargs ../../app.exe

			rc=$? 

			if [ $rc -ne 0 ] && diff -Z "../../func_tests/data/neg_${num}_out.txt" "../../func_tests/data/out.txt"; then 
